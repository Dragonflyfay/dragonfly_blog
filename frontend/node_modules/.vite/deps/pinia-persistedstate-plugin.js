//#region node_modules/pinia-persistedstate-plugin/dist/pinia-persistedstate-plugin.es.js
/**
...
function createPersistedState(options) {
	...
	return (Context) => {
		...
		const data = getState(`${key}-${store.$id}`, storage);
		data && store.$patch(data);
		store.$subscribe((mutation, state) => {
			logger && logGroup(mutation.storeId, mutation, state);
			setState(`${key}-${mutation.storeId}`, state, storage);
		});
	};
}
//#endregion
